...
import {
  ...
  Loader2, Link2, ExternalLink, X, Plus
} from 'lucide-react';
...
const MEDIA_API = `${API_BASE_URL}/api/superadmin/media`;
...
const MediaLibrary = () => {
  ...
  // Filter state
  const [category, setCategory] = useState(''); // '', image, document, audio, video, other
  ...
  const loadAssets = useCallback(async () => {
    setLoading(true);
    try {
      const url = `${MEDIA_API}?page=${page}&limit=12&category=${category}&search=${encodeURIComponent(search)}&branch=${selectedBranch}`;
  ...
  }, [page, category, search, selectedBranch, fetchAPI, toast]);
  ...
  useEffect(() => {
    setPage(1);
  }, [category, search, selectedBranch]);

  useEffect(() => {
    loadAssets();
  }, [loadAssets]);

  // Determine Category based on MIME
  ...
  const handleCopyLink = (resolvedUrl) => {
    ...
    toast.success('Temporary direct link copied to clipboard!');
  };
      ...
      <div className="space-y-6 pb-10">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-gray-400 font-bold mt-1">
              Store and manage digital campaign banners, certificates templates, reports, and documentation
            </p>
          </div>
          <div>
            <button
        ...
        <div className="bg-white border border-gray-100 p-4 rounded-2xl flex flex-col md:flex-row items-center gap-4 shadow-sm">
          ...
          <select
            ...
            className="w-full md:w-48 px-3 py-2 border border-gray-200 rounded-xl text-xs font-bold bg-transparent text-gray-600 outline-none cursor-pointer"
          >
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-4">
            ...
            <p className="text-xs text-gray-400 font-bold">Querying Cloud storage catalog...</p>
          </div>
        ) : assets.length === 0 ? (
          <div className="bg-white border border-gray-100 rounded-3xl py-20 text-center text-gray-400 font-semibold shadow-sm">
            No media assets found matching query criteria.
          </div>
        ) : (
            ...
            {assets.map(asset => (
              ...
              >
                ...
                <div className="p-4 space-y-1 text-left">
                  ...
                  {asset.branch && (
                    <p className="text-[9px] text-[#1B5E20] font-bold mt-1 bg-green-50 border border-green-100 px-1.5 py-0.5 rounded inline-block">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/55 overflow-y-auto no-scrollbar">
          <div className="w-full max-w-3xl bg-white border border-gray-100 shadow-2xl relative rounded-3xl p-6 md:p-8 space-y-5 max-h-[95vh] overflow-y-auto no-scrollbar">
            ...
            <div className="flex justify-center bg-gray-50 p-4 rounded-2xl border border-gray-100 min-h-[30vh]">
              ...
              {previewAsset.category !== 'image' && previewAsset.category !== 'video' && previewAsset.category !== 'audio' && previewAsset.mimeType !== 'application/pdf' && (
                <div className="flex flex-col items-center justify-center py-20 text-center space-y-4">
                  {getCategoryIcon(previewAsset.category)}
                  <p className="text-xs text-gray-400 font-bold">
                    Direct viewer preview not supported for this mime format. Please use the "Open Link" option above.
                  </p>
                </div>
              )}
            </div>
