...
import { Contact, Search, ShieldAlert, Loader2, Printer, CheckCircle, Award, Heart, HelpCircle } from 'lucide-react';
...
const IDCards = () => {
  ...
  const fetchUsers = useCallback(async (role = roleType, searchQ = searchQuery) => {
    ...
    try {
      let endpoint = '';
      if (role === 'Admin') endpoint = `${API_BASE_URL}/api/superadmin/admins`;
      else if (role === 'Member') endpoint = `${API_BASE_URL}/api/superadmin/members`;
      ...
      console.error(err);
      toast.error('Failed to load users for card generation');
    } finally {
      setLoading(false);
    }
  }, [token, roleType, searchQuery, toast]);
  ...
  const getUserDetails = () => {
    ...
    let branchName = selectedUser.branch?.name || 'Central Head Office';
    ...
    <Layout>
      <div className="space-y-6 pb-10">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Render, customize, and batch-print identity credentials for active members and staff</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider">Configuration Controls</h3>

            {/* Target Select */}
            <div className="space-y-1">
              <label className="block text-xs font-bold text-gray-500 uppercase">Target Directory</label>
              <select
                value={roleType}
              ...
              >
                <option value="Member">Members Directory</option>
                <option value="Volunteer">Volunteers Directory</option>
                <option value="Admin">Staff / Branch Administrators</option>
              </select>
            </div>

            {/* User Search Input */}
            <div className="space-y-1">
              <label className="block text-xs font-bold text-gray-500 uppercase">Search Directory Entity</label>
            ...
            <div className="space-y-1">
              <label className="block text-xs font-bold text-gray-500 uppercase">Select Entity Card Target</label>
              <select
            ...
            {/* Theme design */}
            <div className="space-y-1">
              <label className="block text-xs font-bold text-gray-500 uppercase">Select Card Layout Theme</label>
              <select
                value={cardTheme}
          ...
          <div className="lg:col-span-2 flex flex-col items-center justify-center p-6 bg-gray-150 border border-gray-200 rounded-3xl min-h-[400px]">
            {loading ? (
              <Loader2 className="animate-spin text-green-700" size={36} />
            ) : !selectedUser ? (
              <div className="text-center text-gray-400 font-bold text-sm">Select an active user from the configuration sidebar to load ID card preview.</div>
            ) : (
              <div className="space-y-8 w-full max-w-md">
                <h4 className="text-xs font-black text-gray-400 uppercase tracking-widest text-center">Interactive ID Card Previews (85.6mm x 54mm)</h4>

                {/* Print wrapper that matches exactly what handlePrint outputs */}
                <div ref={cardPrintRef} className="flex flex-col gap-6 items-center justify-center">
                  ...
                  <div className={`card-wrapper select-none theme-${cardTheme.toLowerCase()}`} style={{
                    ...
                    <div className="card-header" style={{
                      ...
                      <div>
                        <div className="card-header-logo" style={{ fontSize: '12px', fontWeight: '8px', letterSpacing: '1px', textTransform: 'uppercase' }}>SAVITRAM FOUNDATION</div>
                        <div className="card-header-sub" style={{ fontSize: '8px', opacity: 0.8 }}>Humanitarian Care</div>
                      </div>
