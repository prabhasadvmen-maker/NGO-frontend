...
import {
  FileText, Mail, Users, Check, X, Eye, Trash2, Loader2, ArrowRight, UserCheck, Settings
} from 'lucide-react';
...
const MEMBER_API = `${API_BASE_URL}/api/superadmin/members`;
const VOLUNTEER_API = `${API_BASE_URL}/api/superadmin/volunteers`;
...
const Forms = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      toast.error('Failed to retrieve form submissions');
    } finally {
      setLoading(false);
    }
  ...
  const handleDeleteQuery = async (id) => {
    if (!window.confirm('Delete query?')) return;
  ...
  const handleDeleteMember = async (id) => {
    if (!window.confirm('Delete this pending application?')) return;
    const data = await fetchAPI(`${MEMBER_API}/${id}`, { method: 'DELETE' });
    if (data && data.success) {
  ...
  const handleDeleteVolunteer = async (id) => {
    if (!window.confirm('Delete this pending registration?')) return;
    const data = await fetchAPI(`${VOLUNTEER_API}/${id}`, { method: 'DELETE' });
    if (data && data.success) {
      ...
      <div className="space-y-6 pb-10">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 font-bold mt-1">
            Review and audit contact inquiries, membership applications, and volunteer registrations
          </p>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-gray-500" style={{ borderColor: '#E0E0E0' }}>
                    ...
                    <th className="px-4 py-3.5 text-xs font-bold uppercase text-left">Branch</th>
                ...
                <tbody>
                  {activeTab === 'contact' && (
                    contactQueries.length === 0 ? (
                      <tr><td colSpan="7" className="text-center py-10 text-gray-400 font-semibold">No queries found.</td></tr>
                    ) : (
                      contactQueries.map((q, idx) => (
                        <tr key={q._id} className="border-b last:border-0 hover:bg-gray-50 transition-colors" style={{ borderColor: '#F0F0F0' }}>
                          ...
                          <td className="px-4 py-4 text-xs font-bold text-gray-500">{q.branch?.name || 'Global HQ'}</td>
                            ...
                            <div className="relative inline-block text-left">
                              ...
                              {openMenuId === q._id && (
                                <div className="absolute right-0 top-8 z-20 bg-white border border-gray-100 rounded-xl shadow-xl w-32 py-1" onClick={(e) => e.stopPropagation()}>
                                  ...
                                  <button
                                    onClick={() => { handleDeleteQuery(q._id); setOpenMenuId(null); }}
                  ...
                  {activeTab === 'membership' && (
                    pendingMembers.length === 0 ? (
                      <tr><td colSpan="7" className="text-center py-10 text-gray-400 font-semibold">No pending memberships.</td></tr>
                    ) : (
                  ...
                  {activeTab === 'volunteer' && (
                    pendingVolunteers.length === 0 ? (
                      <tr><td colSpan="7" className="text-center py-10 text-gray-400 font-semibold">No pending volunteer registrations.</td></tr>
                    ) : (
                      pendingVolunteers.map((v, idx) => (
                        <tr key={v._id} className="border-b last:border-0 hover:bg-gray-50 transition-colors" style={{ borderColor: '#F0F0F0' }}>
                          ...
                          <td className="px-4 py-4 text-xs font-bold text-gray-500">{v.branch?.name || 'Global HQ'}</td>
                          <td className="px-4 py-4">
                            <span className="px-2 py-0.5 rounded text-[10px] font-bold border bg-orange-50 text-orange-700 border-orange-200">Pending</span>
                          </td>
          ...
          <div className="w-full max-w-xl bg-white border border-gray-100 shadow-2xl relative rounded-3xl p-6 md:p-8 space-y-5 max-h-[90vh] overflow-y-auto no-scrollbar">
            <div className="flex items-center justify-between pb-2 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5 font-bold font-mono">ID: {selectedItem._id}</p>
              </div>
            ...
            {activeTab === 'contact' && (
              <div className="space-y-4">
                ...
                <div className="flex justify-between pt-3 border-t border-gray-100">
                  <button onClick={() => handleDeleteQuery(selectedItem._id)} className="px-4 py-2.5 bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 rounded-xl text-xs font-bold cursor-pointer">
                    Delete
                  </button>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <span className="text-xs font-bold text-gray-700">{selectedItem.gender || 'Not specified'}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] font-bold text-gray-400 uppercase">Occupation</span>
                    <span className="text-xs font-bold text-gray-700">{selectedItem.occupation || 'N/A'}</span>
                  </div>
                </div>
                <div>
                ...
                <div className="flex justify-between pt-1">
                  <button onClick={() => handleDeleteMember(selectedItem._id)} className="text-red-500 hover:text-red-700 text-xs font-bold bg-transparent border-0 cursor-pointer">
                    Remove Application Record
                  </button>
                  <button onClick={() => setSelectedItem(null)} className="text-gray-400 hover:text-gray-600 text-xs font-bold bg-transparent border-0 cursor-pointer">
                    Cancel
                  </button>
                </div>
              </div>
            ...
            {activeTab === 'volunteer' && (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <span className="block text-[10px] font-bold text-gray-400 uppercase">Skills</span>
                    <span className="text-xs font-bold text-gray-700">{selectedItem.skills?.join(', ') || 'None'}</span>
                  </div>
                  <div>
                    <span className="block text-[10px] font-bold text-gray-400 uppercase">DOB</span>
                    <span className="text-xs font-bold text-gray-700">{selectedItem.dateOfBirth ? new Date(selectedItem.dateOfBirth).toLocaleDateString() : 'N/A'}</span>
                  </div>
                </div>
                <div className="flex gap-3 pt-4 border-t border-gray-100">
                  <button
                ...
                <div className="flex justify-between pt-1">
                  <button onClick={() => handleDeleteVolunteer(selectedItem._id)} className="text-red-500 hover:text-red-700 text-xs font-bold bg-transparent border-0 cursor-pointer">
