...
import { 
  ...
  ShieldCheck, Camera, FileText, Plus, RefreshCw, CheckCircle, XCircle
} from 'lucide-react';
...
const ActionMenu = ({ beneficiary, onView, onEdit, onDelete, onApprove, onReject }) => {
      ...
      {open && (
        <div className="absolute right-0 mt-1 w-44 rounded-xl border border-gray-100 bg-white shadow-lg z-10 overflow-hidden">
          ...
          {beneficiary.status === 'Pending' && (
            <>
              <button onClick={() => { onApprove(beneficiary._id); setOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                <CheckCircle size={14} className="text-green-500" /> Verify/Approve
              </button>
              <button onClick={() => { onReject(beneficiary._id); setOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
...
const BeneficiariesList = () => {
  ...
  // Fetch branches (Fixed path to call '/branches' route instead of '/public/branches')
  ...
  const fetchStats = useCallback(async () => {
    try {
      const res = await fetch(`${API}/admin/beneficiaries/stats`, { headers });
      const data = await res.json();
  ...
  const fetchBeneficiaries = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/admin/beneficiaries?${queryParams}`, { headers });
      const data = await res.json();
      if (data.success) {
  ...
  const uploadPhoto = (file, isEdit = false) => {
    return new Promise((resolve, reject) => {
      ...
      fetch(`${API}/admin/beneficiaries/upload-url?fileName=${encodeURIComponent(fileName)}&contentType=${encodeURIComponent(contentType)}`, {
        ...
        .then(data => {
          ...
          xhr.onload = () => {
            ...
            else reject(new Error('S3 uploader direct upload failed'));
          };
  ...
  const handleApproveRequest = async (id) => {
    if (!window.confirm('Verify and approve this beneficiary registry?')) return;
    try {
      const res = await fetch(`${API}/admin/beneficiaries/${id}/approve-request`, {
        method: 'POST',
        headers
      });
      const data = await res.json();
  ...
  const handleRejectRequest = async (id) => {
    if (!window.confirm('Reject this beneficiary registry request?')) return;
    try {
      const res = await fetch(`${API}/admin/beneficiaries/${id}/reject-request`, {
        method: 'POST',
        headers
      });
      const data = await res.json();
      if (data.success) {
        toast.success(data.message);
        setBeneficiaries(prev => prev.map(b => b._id === id ? { ...b, status: 'Inactive' } : b));
        fetchStats();
      } else {
        toast.error(data.message);
      }
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${API}/admin/beneficiaries/${editBeneficiary._id}`, {
        method: 'PUT',
        headers,
        body: JSON.stringify(bodyData)
      });
  ...
  const handleDelete = async () => {
    try {
      const res = await fetch(`${API}/admin/beneficiaries/${deleteBeneficiary._id}`, { 
        method: 'DELETE', 
        headers 
      });
      const data = await res.json();
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">Manage and track registered aid recipients and support logs</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { label: 'My Registered', value: stats.totalBeneficiaries, color: COLORS.primary, sub: 'All profiles added' },
            { label: 'Active Recipients', value: stats.activeBeneficiaries, color: COLORS.success, sub: 'Verified & active' },
            { label: 'Pending Verification', value: stats.pendingBeneficiaries, color: '#D29C00', sub: 'Awaiting checks' },
            { label: 'Inactive / Suspended', value: stats.inactiveBeneficiaries, color: COLORS.danger, sub: 'On hold' },
          ].map((card, idx) => (
            <div 
              key={idx} 
        ...
        >
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-gray-400 gap-2">
              ...
              <p className="font-semibold text-sm">No beneficiaries registered by you yet</p>
            </div>
          ) : (
                    ...
                    <tr key={beneficiary._id} className="border-b last:border-0 hover:bg-gray-50 transition-colors" style={{ borderColor: '#F0F0F0' }}>
                      ...
                      <td className="px-3.5 py-3">
                        <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${
                          ...
                          beneficiary.status === 'Pending' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-600'
                        }`}>
                          {beneficiary.status}
                        </span>
                      </td>
              ...
              <div className="text-center md:text-left space-y-1">
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-2 pt-1">
                  <span className={`px-3 py-0.5 rounded-full text-xs font-bold ${
                    viewBeneficiary.status === 'Active' ? 'bg-green-100 text-green-700' :
                    viewBeneficiary.status === 'Pending' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-750'
                  }`}>
                    {viewBeneficiary.status}
                  </span>
