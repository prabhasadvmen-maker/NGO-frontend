...
import { 
  Users, CheckCircle, XCircle, Search, Loader2, Eye, X, Landmark, Phone, Mail, MapPin, User, Calendar, FileText, Settings
} from 'lucide-react';
...
const ActionMenu = ({ beneficiary, onView, onApprove, onReject }) => {
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 mt-1 w-44 rounded-xl border border-gray-100 bg-white shadow-lg z-10 overflow-hidden">
          ...
          <button onClick={() => { onReject(beneficiary._id); setOpen(false); }} className="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 transition-colors">
...
const BeneficiaryVerification = () => {
  ...
  const fetchPending = useCallback(async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        toast.error(data.message || 'Failed to fetch pending beneficiaries');
      }
    } catch (err) {
      console.error(err);
      toast.error('Network error loading pending beneficiaries');
    } finally {
      setLoading(false);
    }
  ...
  const handleApprove = async (id) => {
    if (!window.confirm('Verify and approve this beneficiary registration?')) return;
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(data.message || 'Beneficiary verified successfully');
        setBeneficiaries(prev => prev.filter(b => b._id !== id));
      } else {
        toast.error(data.message);
      }
    } catch {
      toast.error('Server error approving beneficiary');
    }
  };

  const handleReject = async (id) => {
    if (!window.confirm('Reject this beneficiary registration request?')) return;
    try {
      ...
      const data = await res.json();
      if (data.success) {
        toast.success(data.message || 'Beneficiary request rejected');
        setBeneficiaries(prev => prev.filter(b => b._id !== id));
      } else {
        toast.error(data.message);
      }
    } catch {
      toast.error('Server error rejecting beneficiary');
    }
  };
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-400 mt-0.5">Audit and approve registered beneficiaries to authorize aid delivery</p>
        </div>
        ...
        >
          {loading ? (
            <div className="flex flex-col items-center justify-center py-24 gap-3">
              ...
              <p className="text-sm font-semibold text-gray-400">Loading pending requests...</p>
            </div>
          ) : beneficiaries.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-20 text-gray-400 gap-2">
              <Users size={44} className="opacity-30" />
              <p className="font-semibold text-sm">No pending beneficiaries awaiting verification</p>
            </div>
          ) : (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b" style={{ borderColor: '#E0E0E0' }}>
                    {['#', 'ID', 'Recipient Profile', 'Contact', 'Branch', 'Category / Needs', 'Status', 'Verification Action'].map(h => (
      ...
      {viewBeneficiary && (
        <Modal onClose={() => setViewBeneficiary(null)}>
          <ModalHeader title="Beneficiary Registration Request Details" onClose={() => setViewBeneficiary(null)} />
