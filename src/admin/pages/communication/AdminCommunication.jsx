...
import {
  ...
  Trash2, Eye, ShieldAlert, CheckCircle, Info, Settings, X
} from 'lucide-react';
...
const COMM_API = `${API_BASE_URL}/api/admin/communication`;

const AdminCommunication = () => {
  const { token } = useAuth();
  const { toast } = useToast();

  const [activeSubTab, setActiveSubTab] = useState('send'); // send, logs
  ...
  const fetchAPI = useCallback(async (url, options = {}) => {
    try {
      ...
      console.error('Admin Communication API error:', err);
      return null;
    }
  }, [token, toast]);
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      if (json && json.success) {
        toast.success(json.message || 'Notification broadcast logged successfully under this branch!');
      ...
      <div className="space-y-6 pb-10">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 font-bold mt-1">
            Send bulk announcements scoped strictly to this branch's members, volunteers, or donors lists
          </p>
        </div>
        ...
        {activeSubTab === 'send' && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white border border-gray-100 p-6 rounded-3xl shadow-sm text-left">
              <h3 className="text-base font-extrabold text-gray-800 mb-4">Compose Branch Broadcast</h3>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="All">All Scoped Contacts</option>
                    </select>
                  </div>
                </div>
                ...
                {form.type === 'Email' && (
                  <div>
                    ...
                    <input
                      type="text"
                      required
                      placeholder="e.g. Luckow Branch Drive Notice"
                      value={form.subject}
            ...
            <div className="bg-white border border-gray-100 p-6 rounded-3xl shadow-sm space-y-4 text-left">
              <h3 className="text-base font-extrabold text-gray-800 flex items-center gap-1.5"><Info className="text-yellow-600" size={20} /> Branch Sandbox Mode</h3>
              <p className="text-xs text-gray-550 leading-relaxed font-semibold">
                This broadcast portal updates internal notification logs and simulates delivery parameters.
              </p>
              <div className="p-3 bg-green-50/50 border border-green-150 rounded-2xl flex items-start gap-2">
                ...
                <span className="text-[11px] text-green-800 leading-normal font-bold">
                  Branch dispatch logs are synced immediately to the <strong>Dispatch Audits</strong> tab for local tracking.
                </span>
              </div>
            </div>
          </div>
        ...
        {activeSubTab === 'logs' && (
          loading ? (
            ...
            <div className="bg-white border border-gray-100 rounded-3xl py-20 text-center text-gray-400 font-semibold shadow-sm">
              No branch broadcast logs found.
            </div>
          ) : (
          ...
          <div className="w-full max-w-xl bg-white border border-gray-100 shadow-2xl relative rounded-3xl p-6 md:p-8 space-y-5 max-h-[90vh] overflow-y-auto no-scrollbar">
            <div className="flex items-center justify-between pb-2 border-b border-gray-100">
              <div className="text-left">
                ...
                <p className="text-xs text-gray-400 mt-0.5 font-bold font-mono">ID: {viewingItem._id}</p>
              </div>
            ...
            <div className="space-y-4 text-left">
              ...
              <div>
                <span className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Audience Scope</span>
                <div className="p-3 bg-green-50 text-green-800 border border-green-150 rounded-xl text-xs font-bold flex items-center gap-1.5">
                  Target: {viewingItem.recipientType} ({viewingItem.recipientsCount} total recipients in branch drive).
                </div>
              </div>
            </div>
