  ...
  return (
    <Layout>
      <div className="space-y-6 pb-10">
        ...
        {/* Title */}
        <div>
          <h1 className="text-3xl font-extrabold text-gray-800 tracking-tight">Donation Receipts</h1>
          <p className="text-sm text-gray-400 font-semibold mt-1">Generate, print, and download receipt invoices for completed transactions</p>
        </div>
        ...
        <div className="rounded-3xl overflow-hidden bg-white border border-gray-100 shadow-xl">
          {loading ? (
            ...
            <div className="text-center py-20 text-gray-400 font-semibold text-sm">No completed donations found to generate receipts.</div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-gray-50 border-gray-200 text-xs font-bold text-gray-500 uppercase tracking-wider">
                ...
                <tbody className="divide-y divide-gray-100">
                  {donations.map(donation => (
                    <tr key={donation._id} className="hover:bg-gray-50/30 transition-colors">
                      <td className="px-6 py-4 font-mono font-bold text-green-800">{donation.receiptNumber}</td>
                      <td className="px-6 py-4 font-bold text-gray-800">{donation.donorName}</td>
                      <td className="px-6 py-4 font-black text-gray-800">₹{donation.amount.toLocaleString('en-IN')}</td>
                      <td className="px-6 py-4 text-gray-600 font-semibold">
                        {new Date(donation.donationDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                      </td>
                      <td className="px-6 py-4 font-semibold text-gray-600">{donation.purpose}</td>
                      <td className="px-6 py-4 capitalize font-semibold text-gray-600">{donation.paymentMethod.replace('_', ' ')}</td>
                      <td className="px-6 py-4">
                        <button
                          onClick={() => handleOpenReceipt(donation)}
                          className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg font-bold text-xs cursor-pointer border-0 hover:bg-gray-200 transition-colors"
                        >
                          <Eye size={12} />
                          View Receipt
                        </button>
