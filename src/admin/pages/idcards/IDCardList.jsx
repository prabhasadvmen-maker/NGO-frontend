...
const IDCardList = () => {
  ...
  const fetchUsers = useCallback(async (role = roleType, searchQ = searchQuery) => {
    ...
    setLoading(true);
    try {
      const prefix = user?.role === 'super_admin' ? 'superadmin' : 'admin';
      let endpoint = '';
      if (role === 'Member') endpoint = `${API_BASE_URL}/api/${prefix}/members`;
      else if (role === 'Volunteer') endpoint = `${API_BASE_URL}/api/${prefix}/volunteers`;
      ...
      if (data.success) {
        ...
        // Filter by branch if user has a branch assigned, but allow entries with no branch assigned (null/undefined)
        if (user && user.branch) {
          ...
          filtered = filtered.filter(u => {
            ...
            return !uBranchId || uBranchId === userBranchId;
          });
        }
  ...
  const getUserDetails = () => {
    ...
    let name = selectedUser.fullName || selectedUser.name || 'N/A';
    let email = selectedUser.email;
    let uniqueId = selectedUser.memberId || selectedUser.volunteerId || selectedUser.membershipId || selectedUser._id.substring(18).toUpperCase();
    ...
    let phone = selectedUser.mobileNumber || selectedUser.phone || 'N/A';
    let status = selectedUser.isActive || selectedUser.status === 'Approved' || selectedUser.status === 'Active' ? 'ACTIVE' : 'INACTIVE';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <div className="space-y-1">
              ...
              <select
                value={roleType}
                onChange={(e) => { setRoleType(e.target.value); setSearchQuery(''); setUsersList([]); }}
          ...
          {/* Preview Panel */}
          <div className="lg:col-span-2 flex flex-col items-center justify-center p-6 bg-gray-55 border border-gray-200 rounded-3xl min-h-[400px]">
            {loading ? (
                ...
                <div ref={cardPrintRef} className="flex flex-col gap-6 items-center justify-center">
                  ...
                  <div className={`card-wrapper select-none theme-${cardTheme.toLowerCase()}`} style={{
                    ...
                    <div className="card-header" style={{
                      ...
                      backgroundColor: cardTheme === 'Corporate' ? '#0d47a1' : cardTheme === 'Life' ? '#311b92' : '#1B5E20',
                      color: '#fff'
                    }}>
                      <div>
                    ...
                    <div className="card-body" style={{ display: 'flex', padding: '12px', boxSizing: 'border-box', gap: '12px' }}>
                      <img 
                        ...
                        style={{
                          ...
                          borderRadius: '6px',
                          objectFit: 'cover',
                          border: `2px solid ${cardTheme === 'Corporate' ? '#0d47a1' : cardTheme === 'Life' ? '#311b92' : '#1B5E20'}`,
                      ...
                      <div className="profile-info" style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', fontSize: '11px' }}>
                        <div>
                          ...
                          <p className="info-id" style={{ fontSize: '11px', fontWeight: '800', margin: '2px 0 6px 0', color: cardTheme === 'Corporate' ? '#0d47a1' : cardTheme === 'Life' ? '#311b92' : '#1B5E20' }}>ID: {uDetails.uniqueId}</p>
                        </div>
